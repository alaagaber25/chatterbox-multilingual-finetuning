...
)  # The actual module we want to finetune
...
class S3GenModelArguments:
    model_name_or_path: Optional[str] = field(
        default=None, metadata={"help": "Path to base Chatterbox model"}
    )
    local_model_dir: Optional[str] = field(
        default=None, metadata={"help": "Path to local Chatterbox model directory"}
    )
    cache_dir: Optional[str] = field(
        default=None, metadata={"help": "HF cache directory"}
    )
    ...
    freeze_s3_tokenizer: bool = field(
        default=True, metadata={"help": "Freeze S3Gen's internal S3Tokenizer."}
    )
...
class S3GenDataArguments:
    dataset_name: Optional[str] = field(
        default=None, metadata={"help": "HF Dataset name"}
    )
    dataset_config_name: Optional[str] = field(
        default=None, metadata={"help": "HF Dataset config name"}
    )
    train_split_name: str = field(
        default="train", metadata={"help": "Train split name"}
    )
    eval_split_name: Optional[str] = field(
        default="validation", metadata={"help": "Eval split name"}
    )
    audio_column_name: str = field(
        default="audio", metadata={"help": "Audio column in dataset"}
    )
    # No text column needed for S3Gen training directly, but might be in dataset

    max_speech_token_len: int = field(
        default=750, metadata={"help": "Max S3 speech tokens for target sequence."}
    )
    ...
    eval_split_size: float = field(
        default=0.01, metadata={"help": "Eval split fraction"}
    )  # Increased default
    ignore_verifications: bool = field(
        default=False, metadata={"help": "Ignore dataset verifications."}
    )
    ...
    def __getitem__(self, idx) -> Optional[Dict[str, torch.Tensor]]:
        ...
        try:
            ...
            speech_token_len = torch.tensor(
                target_s3_tokens.size(0), dtype=torch.long, device="cpu"
            )
            ...
            if target_mel.size(0) != 2 * target_s3_tokens.size(0) and target_mel.size(
                ...
                speech_token_len = torch.tensor(
                    target_s3_tokens.size(0), dtype=torch.long
                )
                ...
                target_s3_tokens = F.pad(
                    target_s3_tokens, (0, pad_size), value=0
                )  # Pad with 0
                # speech_token_len remains the original length before padding for this case

        except Exception as e:
            logger.error(
                f"Item {idx}: Error tokenizing target speech: {e}", exc_info=True
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Item {idx}: Error getting speaker_embedding: {e}", exc_info=True
            )
        ...
        if (
            ...
            prompt_mel = torch.zeros(
                max_flow_prompt_token_len * 2, target_mel.size(1), dtype=torch.float
            )  # (T_prompt_mel, F)
            ...
            try:
                ...
                max_flow_prompt_token_len = getattr(
                    self.s3gen_model.flow.encoder, "prompt_token_max_len", 75
                ...
                if prompt_mel.size(0) // 2 < prompt_s3_tokens.size(0):
                    ...
                    prompt_s3_token_len = torch.tensor(
                        prompt_s3_tokens.size(0), dtype=torch.long
                    )

            except Exception as e:
                logger.error(
                    f"Item {idx}: Error processing prompt features: {e}", exc_info=True
                )
                return None
...
class S3GenDataCollator:
    ...
    def __call__(
        self, features: List[Optional[Dict[str, torch.Tensor]]]
    ) -> Dict[str, Any]:
...
class S3GenFlowForFineTuning(torch.nn.Module):
    ...
    def forward(
        ...
        h_encoded_full, _ = self.flow_model.encoder(
            input_token_embed, full_input_token_lens
        )
        ...
        if T_prompt_mel_collator > T_target_mel_collator:
            ...
            padded_prompt_mels_for_cond = F.pad(
                original_prompt_mels, (0, padding_size), mode="constant", value=0
            )
        ...
        if slice_start_idx >= mu_full_for_cfm.size(2):
            ...
            if current_raw_target_slice_len < num_target_mel_frames_collator:
                ...
                mu_conditioning_for_target_mels = F.pad(
                    mu_target_raw_slice, (0, padding_needed)
                )
    ...
    def make_pad_mask(
        self, lengths: torch.Tensor, max_len: Optional[int] = None
    ) -> torch.Tensor:
        # ... (same as before) ...
...
def main():
    ...
    logger.info(
        "Loading base ChatterboxMultilingualTTS model to extract S3Gen components..."
    )
    ...
    if model_args.local_model_dir:
        chatterbox_model = ChatterboxMultilingualTTS.from_local(
            ckpt_dir=model_args.local_model_dir, device="cpu"
        )
    else:
        repo_to_download = (
        ...
        chatterbox_model = ChatterboxMultilingualTTS.from_local(
            ckpt_dir=download_dir, device="cpu"
        )
    ...
    if data_args.dataset_name:
        ...
        raise ValueError(
            "S3Gen finetuning currently requires a Hugging Face dataset_name."
        )  # Simpler for now

    train_dataset = S3GenFineTuningDataset(
        data_args, s3gen_model_to_finetune, train_hf_dataset
    )
    eval_dataset = None
    if eval_hf_dataset and training_args.do_eval:
        eval_dataset = S3GenFineTuningDataset(
            data_args, s3gen_model_to_finetune, eval_hf_dataset
        )
    ...
    trainer_instance = Trainer(
        ...
        compute_metrics=compute_metrics_s3gen
    ...
    if training_args.do_train:
        ...
        logger.info(
            "Saving finetuned S3Gen (flow part) model weights for ChatterboxMultilingualTTS..."
        )
